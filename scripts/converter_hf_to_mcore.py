...
def convert_checkpoint_from_transformers_to_megatron_bailingv2moe(
    ...
    for layer_idx, (layer, hf_layer) in enumerate(
        ...
        # LayerNorms
        try:
            weight = hf_layer.input_layernorm.weight
            numel += safe_copy(weight, layer.input_layernorm.weight)
        except AttributeError:
            pass

        try:
            weight = hf_layer.post_attention_layernorm.weight
            numel += safe_copy(weight, layer.pre_mlp_layernorm.weight)
        except AttributeError:
            pass
        ...
        # Determine if this is a MoE layer (i.e., has 'gate' in mlp)
        ...
        if is_moe_layer:
            ...
            layer.mlp.linear_fc2.weight.copy_(fc2_weight)
            numel += fc1_weight.numel() + fc2_weight.numel()
